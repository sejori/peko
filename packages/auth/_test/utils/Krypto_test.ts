...
Deno.test("UTIL: Krypto", async (t) => {
  const hmacKrypto = new Krypto("SUPER_SECRET_KEY_123");
  ...
  await t.step("hash creates repeatable hash", async () => {
    const hmac_hash1 = await hmacKrypto.hash(str);
    const hmac_hash2 = await hmacKrypto.hash(str);
  ...
  await t.step("HMAC sign creates valid jwt", async () => {
    hmac_token = await hmacKrypto.sign(inputPayload);

    const [hmac_b64Header, hmac_b64Payload, hmac_b64Signature] =
      hmac_token.split(".");
  ...
  await t.step("HMAC verify return valid jwt payload", async () => {
    const hmac_payload = await hmacKrypto.verify(hmac_token);
  ...
  await t.step("RSA sign creates valid jwt from CryptoKeyPair", async () => {
    ...
    const [rsa_b64Header, rsa_b64Payload, rsa_b64Signature] =
      rsa_token.split(".");
  ...
  await t.step(
    "RSA verify return valid jwt payload from CryptoKeyPair",
    async () => {
  ...
  await t.step("RSA sign creates valid jwt from key/cert files", async () => {
    ...
    const [rsa_b64Header_cert, rsa_b64Payload_cert, rsa_b64Signature_cert] =
      rsa_token_cert.split(".");
  ...
  await t.step(
    "RSA verify return valid jwt payload from key/cert files",
    async () => {
      const rsa_from_cert_payload = await rsaPublicCrypto_fromCert.verify(
        rsa_token_cert
      );
      assert(
        JSON.stringify(rsa_from_cert_payload) == JSON.stringify(inputPayload)
      );
    }
  );
});
