...
import blankTemplate from "./exampleSrc/blankTemplate.js";
...
Peko.setConfig({
  ...
  errorHandler: async (statusCode, _request) =>
...
const pageRoutes = [
  ...
  },
  {
    ...
    render: (app) => renderToString(app, null, null),
    moduleURL: new URL("./exampleSrc/pages/Navigation.js", import.meta.url),
    customTags: {
      style: style,
      elementId: "nav",
      hydrationScript: `
            <script type="module">
                import { hydrate } from "https://npm.reversehttp.com/preact,preact/hooks,htm/preact,preact-render-to-string";
                import Home from "/exampleSrc/pages/Navigation.js";
                hydrate(Navigation(), document.getElementById(${"nav"}))
            </script>
        `,
    },
    cacheLifetime: 6000,
...
const customRoutes = [
  // must be PekoRoute type (see types.ts)
  {
    ...
    method: "POST",
    handler: (request) =>
      new Response(`Parrot sqwarks: ${JSON.stringify(request.body)}`),
  },
];
