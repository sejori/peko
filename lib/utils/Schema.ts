...
export class Schema {
  ...
  private generateOperationFields(type: "query" | "mutation"): string {
    return this.operations
      ...
      .map((operation) => {
        const args = Object.entries(operation.config.input.config.fields)
          .map(([name, field]) => `${name}: ${this.generateFieldString(field)}`)
          .join(", ");
        const outputType = Array.isArray(operation.config.output)
          ? `[${operation.config.output[0].name}]!`
          : `${operation.config.output.name}!`;
  ...
  private generateDTOs(): string {
    ...
    this.operations.forEach((operation) => {
      DTOs.add({ dtoType: "input", dto: operation.config.input });
      DTOs.add({ dtoType: "type", dto: operation.config.output });
    });

    console.log(DTOs.values());
