...
export class Schema {
  ...
  constructor(
    ...
    this.operations = Object.values(routes);  
  }
  ...
  private generateModels(): string {
    ...
    this.operations.forEach((operation) => {
      ...
      if (operation.config.args) {
        models.add({ 
          type: "input", 
          model: operation.config.args,
          defaultName: `${operation.path}Input`
        });
      }

      // add operation result types (if model constructors)
      if ("schema" in operation.config.type) models.add({ 
        type: "type", 
        model: operation.config.type as ModelInterface,
        defaultName: `${operation.path}Type`
      });
    });
  ...
  private generateModelFields(input: {
    ...
    return `${input.type} ${model.name || input.defaultName} {\n${fieldsString}\n}\n\n`;
  }
